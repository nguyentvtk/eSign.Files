...
router.post('/approve', authenticate, requireRole('Admin', 'Quản lý'), async (req, res) => {
  ...
  if (doc.file_url?.startsWith('/uploads')) {
    ...
    // Tải tạm từ Dropbox để stamp
    try {
      const resp = await fetch(doc.file_url);
      if (resp.ok) {
        const buf = Buffer.from(await resp.arrayBuffer());
        const tempPath = pathMod.join(config_.upload.dir, 'temp', `${Date.now()}_${doc.file_name}`);
        if (!fs.existsSync(pathMod.dirname(tempPath))) fs.mkdirSync(pathMod.dirname(tempPath), { recursive: true });
  ...
  if (localPath && fs.existsSync(localPath)) {
    try {
      ...
      // Upload file đã ký lên cùng folder Dropbox với file gốc
      const subfolder = `${doc.ma_doc}_${doc.dropbox_path ? '' : 'signed'}/da-ky`.replace(/\/+/g, '/');
